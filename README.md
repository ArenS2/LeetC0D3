...
- File code đó tương đối dài nên mình sẽ tóm tắt những ý quan trọng về nội dung của file code đó:
	- Đầu tiên tác giả tạo sẵn 1 biến số nguyên `p` rất lớn (2049 bits) 
	```sh
...
- Về mặt lý thuyết ta đã giải xong bài toán, nhưng trong code đề bài cho nó chặt chẽ hơn: chỉ cho phép độ dài của `m`, `n` ở hệ hex <= 512 kí tự, nhưng với kết quả ta vừa tính được, độ dài của cả `m` và `n` đều là 513.
```sh
try:
	print('Username:') 
	username = input() 
