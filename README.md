...
- File code đó tương đối dài nên mình sẽ tóm tắt những ý quan trọng về nội dung của file code đó:
	...
	```
	- Bài toán đặt ra ở đây sẽ là: chúng ta sẽ tìm và nhập 2 biến `username` & `password` (tạm thay bằng `m`, `n` tương ứng) sao cho: (x<sup>m</sup> % p)\*(y<sup>n</sup> % p) % p == `server_hash`. 
